...
export default function useFilterTemplates({ notify, authLoading, filterSetters, columnVisibility }) {
  ...
  const applyTemplateLocal = async (template) => {
    try {
      ...
      s.setLeadStatus(template.filters.status?.[0] || 'Lead Status');
      s.setCallerFilter(template.filters.assignee?.[0] || 'All Callers');
      ...
      s.setCampaignFilter(template.filters.campaign?.[0] || 'All Campaigns');
