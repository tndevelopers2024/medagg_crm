...
export default function useLeadAnalytics({
  ...
  const mergedAnalyticsFilters = useMemo(() => {
    ...
    if (leadStatus !== "Lead Status") {
      af.push({ id: "page_status", type: "leadStatus", operator: "is", value: leadStatus });
    }
    if (callerFilter !== "All Callers") {
      af.push({ id: "page_caller", type: "assignee", operator: "is", value: callerFilter === "Unassigned" ? "Unassigned" : callerFilter });
    }
    if (source !== "All Sources") {
      af.push({ id: "page_source", type: "source", operator: "is", value: source });
    }
    if (opdStatus !== "OPD Status") {
      af.push({ id: "page_opd", type: "custom_opd_status", operator: "is", value: opdStatus });
    }
    if (ipdStatus !== "IPD Status") {
      af.push({ id: "page_ipd", type: "custom_ipd_status", operator: "is", value: ipdStatus });
    }
    if (diagnostics !== "Diagnostics") {
      af.push({ id: "page_diag", type: "custom_diagnostic", operator: "is", value: diagnostics });
    }
    if (campaignFilter !== "All Campaigns") {
      af.push({ id: "page_campaign", type: "campaign", operator: "is", value: campaignFilter });
    }
    ...
    if (followupFilter && followupFilter !== "All") {
      af.push({ id: "page_followup", type: "followUp", operator: "is", value: followupFilter });
    }
    ...
    return af;
  }, [chartDrillFilters, leadStatus, callerFilter, source, opdStatus, ipdStatus, diagnostics, campaignFilter, dateMode, customFrom, customTo, followupFilter, customFieldFilters]);
