...
export default function LeadsManagement() {
  ...
  useEffect(() => {
    if (location.state?.filter) {
      ...
      if (filter.leadStatus) filters.setLeadStatus(filter.leadStatus);
      if (filter.opdStatus) filters.setOpdStatus(filter.opdStatus);
      if (filter.ipdStatus) filters.setIpdStatus(filter.ipdStatus);
      if (filter.campaignFilter) filters.setCampaignFilter(filter.campaignFilter);
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      {/* Sidebar & Floating Actions - Admin only */}
      {isAdmin && (
        <>
          <BulkEditSidebar
            ...
            leadStages={filters.leadStatusOptions.filter(s => s !== 'Lead Status')}
      ...
      <section className="mb-6">
        <LeadFilters
          ...
          caller={filters.callerFilter} setCaller={filters.setCallerFilter} callerOptions={isAdmin ? filters.callerOptions : []}
        ...
        <Card bodyStyle={{ padding: 0 }}>
          ...
          {isAdmin && (
            <div className="p-4 border-t bg-gray-50 flex justify-end">
              <Button
                type="link"
                onClick={() => bulkOps.setShowLocationAssignModal(true)}
              >
                Assign by Location
              </Button>
            </div>
